...
def dump_macro_inc() -> str:
    return "\n".join(
        [
            "/* Auto-generated by leosplit. */",
            ".set noat",
            ".set noreorder",
            ".set gp=64",
            "",
...
def dump_ld_script(metadata: ProjectMetadata, segments: Sequence[SplitSegment]) -> str:
    lines = [
        "/* Auto-generated by leosplit. */",
        "OUTPUT_ARCH(mips)",
        "SECTIONS",
    ...
    for segment in segments:
        lines.extend(
            [
                f"  . = {format_hex(segment.vram)};",
                f"  .seg_{segment.safe_name} {format_hex(segment.vram)} : {{",
                f"    build/obj/{segment.safe_name}.o(.text*)",
                f"    build/obj/{segment.safe_name}.o(.rodata*)",
                f"    build/obj/{segment.safe_name}.o(.data*)",
                f"    build/obj/{segment.safe_name}.o(.bss*)",
                "  }",
            ]
        )
    lines.extend(["}", ""])
...
def dump_makefile(metadata: ProjectMetadata, image_path: str, output_dir: str) -> str:
    rebuilt = f"build/{metadata.basename}.ndd"
    source_image = os.path.relpath(
    ...
    return "\n".join(
        [
            "# Auto-generated by leosplit.",
            "PYTHON ?= python3",
            "LEOSPLIT_BUILD ?= $(PYTHON) -m leosplit.builder.leosplit_build",
            f"BASENAME := {metadata.basename}",
            f"SOURCE_IMAGE := {source_image}",
            f"REBUILT_IMAGE := {rebuilt}",
            "",
            ".PHONY: all rebuild compare clean",
            "",
            "all: rebuild",
            "",
            "rebuild:",
            "\t$(LEOSPLIT_BUILD) . -o $(REBUILT_IMAGE) --base $(SOURCE_IMAGE)",
            "",
            "compare:",
            "\t$(LEOSPLIT_BUILD) . -o $(REBUILT_IMAGE) --base $(SOURCE_IMAGE) --compare $(SOURCE_IMAGE)",
            "",
            "clean:",
            "\trm -rf build",
            "",
            "# Future asm replacement flow can assemble asm/*.s into build/obj/*.o,",
            "# link with the generated .ld script, then place replacement bins in bin/.",
            "",
        ]
    )


def dump_workspace_manifest(
    image_path: str,
    manifest_path: str,
    metadata: ProjectMetadata,
    segments: Sequence[SplitSegment],
    ...
    payload = {
        ...
        "ld_script_path": metadata.ld_script_path,
        "segments": [
            {
                ...
                "lba_start": segment.lba_start,
                "lba_length": segment.lba_length,
                "bin_path": f"bin/{segment.safe_name}.bin",
                "asm_path": f"asm/{segment.safe_name}.s",
                "symbol_path": f"symbols/{segment.safe_name}.sym",
            }
            for segment in segments
        ],
    }
...
def create_workspace(
    ...
    write_text(os.path.join(output_dir, "macro.inc"), dump_macro_inc(), overwrite)
    write_text(os.path.join(output_dir, metadata.ld_script_path), dump_ld_script(metadata, segments), overwrite)
    write_text(os.path.join(output_dir, "Makefile"), dump_makefile(metadata, image_path, output_dir), overwrite)
    write_text(
        os.path.join(output_dir, "leosplit_workspace.json"),
        dump_workspace_manifest(image_path, manifest_path, metadata, segments),
        overwrite,
    )
    return results
